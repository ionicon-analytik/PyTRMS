...
class Step:
    ...
    >>> step = Step("H50", {'DPS_Udrift': 500}, 10, start_delay=2, post_step=None)
    >>> step.set_values
    {'DPS_Udrift': 500}

    Note, that no Automation-numbers can be defined in the step:
    >>> Step("H50", {'AUTO_UseMean': 0}, 10, start_delay=2, post_step=None)
    ...
    def __init__(self, name, set_values, duration, start_delay, post_step):
        self.name = str(name)
        ...
        for key in self.set_values.keys():
            assert not key in Step.protected_keys, "Automation numbers cannot be defined"
